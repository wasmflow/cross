...
pub fn run(
    ...
    docker
        .args(&[
            ...
            &format!(
                "XARGO_HOME=/cross/{}",
                xargo_dir.file_name().unwrap().to_str().unwrap()
            ),
        ])
        .args(&[
            "-e",
            &format!(
                "CARGO_HOME=/cross/{}",
                cargo_dir.file_name().unwrap().to_str().unwrap()
            ),
        ])
    ...
    docker
        ...
        .args(&[
            ...
            &format!(
                "/cross/{}",
                mount_root.file_name().unwrap().to_str().unwrap()
            ),
        ]);
    ...
    docker
        ...
        .args(&[
            ...
            &format!(
                "PATH=$PATH:/cross/{}/bin {:?}",
                sysroot.file_name().unwrap().to_str().unwrap(),
                cmd
            ),
        ])
        .run_and_get_status(verbose)?;
    ...
    docker_command("cp")?
        .args(&[
            "helper:/cross/target",
            &format!("{}", target_dir.parent().unwrap().display()),
        ])
        .run_and_get_status(verbose)?;
